...
int main() {
    ...
    printf_s("Enter the name of the file to create (or press Enter for default '%s'): \n", DEFAULT_FILENAME);
    fflush(stdin);
    ...
    if (fp == NULL) {
        printf_s("Error: could not open output file for operation; Give the name with extension properly!\n");
        system("pause");
        return 1;
    }
    long long total_size;
    char size_input[256];
    printf_s("Enter output file size in bytes (or press Enter to use default size of 1000B): \n");
    fflush(stdin);
    ...
    long long block_count = total_size / BLOCK_SIZE, remainder = total_size % BLOCK_SIZE;
    printf_s("Doing ...!\n");
